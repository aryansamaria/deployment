...
while True:
    ...
    count = {
             ...
             'Name': len(os.listdir(directory+"/Name"))
             }

    row = frame.shape[1]
    col = frame.shape[0]
    cv2.rectangle(frame,(0,40),(500,400),(255,255,255),2)
    ...
    if interrupt & 0xFF == ord('1'):
        cv2.imwrite(os.path.join(directory+'What/' + str(count['What']))+ '.jpg',frame)
    if interrupt & 0xFF == ord('2'):
        cv2.imwrite(os.path.join(directory+'Is/' + str(count['Is']))+ '.jpg',frame)
    if interrupt & 0xFF == ord('3'):
        cv2.imwrite(os.path.join(directory+'Your/' + str(count['Your']))+ '.jpg',frame)
    if interrupt & 0xFF == ord('4'):
        cv2.imwrite(os.path.join(directory+'Name/' + str(count['Name']))+ '.jpg',frame)
